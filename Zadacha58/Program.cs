...
Fill2DIntArrayRandomNumbers(firstMatrix);
Write2DIntArray(firstMatrix);
Fill2DIntArrayRandomNumbers(secondMatrix);
Write2DIntArray(secondMatrix);
...
for (int j = 0; j < numbersResult.GetLength(1); j++)
{
    for (int i = 0; i < numbersResult.GetLength(0); i++)
    {
        ...
        {
            numbersResult[j, i] += firstMatrix[i, k] * secondMatrix[k, j];
        }
    }
}

Write2DIntArray(numbersResult);
